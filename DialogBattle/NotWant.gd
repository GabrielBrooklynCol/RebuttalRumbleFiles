...
# var a = 2
# var b = "text"
var dialog = ["How would you know!? Cat can't resists the Wuviest Duviest bandit that ever Wuvied. That jealousy from you and that excuse i called a gang ticks me off!"]
	...
	if event is InputEventMouseButton and event.pressed:
		if get_visible_characters() > get_total_character_count():
			...
			if page+1 > dialog.size()-1: #The very last dialog will be filler that's intended to be skipped over. Once it's skipped, the dialog box will be closed
				...
				$ResponseNotWantBTN.visible = true
				#page = -2
